#!/bin/bash
# ==============================================================================
# EvalHub Master Orchestrator: Monolithic vLLM Pipeline Implementation
# ==============================================================================
set -euo pipefail
...
CONFIG_FILE="scripts/gemma_pipeline.env"
...
# Utilities
# ------------------------------------------------------------------------------
setup_isolated_vllm_env() {
    local overlay_dir=".vllm_overlay_env"
    
    if [ ! -d "$overlay_dir" ]; then
        echo "[INFO] Gemma 4 mimarisi tespit edildi. Ana ortama (evalhub_env) dokunmadan izole bir overlay ortam oluşturuluyor..."
        # system-site-packages bayrağı: evalhub_env'deki vLLM ve PyTorch'u kullanır, ancak yeni yüklemeleri buraya hapseder.
        python -m venv "$overlay_dir" --system-site-packages
        
        # Sadece bu izole ortama en güncel transformers'ı kuruyoruz. Ana ortam etkilenmez.
        "$overlay_dir/bin/pip" install git+https://github.com/huggingface/transformers.git -U
    else
        echo "[INFO] İzole vLLM overlay ortamı ($overlay_dir) zaten mevcut, kullanılıyor."
    fi
}
...
start_server_and_wait() {
    ...
    # Gemma 4 veya güncel transformers gerektiren modeller için izole ortamı devreye sok
    if [[ "$model_path" == *"gemma-4"* ]]; then
        setup_isolated_vllm_env
        python_bin=".vllm_overlay_env/bin/python"
    fi

    echo "[INFO] Initializing vLLM server for model: $model_path on port $port"
    echo "[INFO] Hardware Strategy: --tensor-parallel-size $tp_size"
    
    # Sunucuyu (gerekirse izole python binary'si ile) başlatıyoruz
    ...
    echo "[INFO] Waiting for vLLM server healthcheck..."
    # vLLM exposes /v1/models as a standard health check endpoint
    while ! curl -s "http://127.0.0.1:$port/v1/models" > /dev/null; do
        if ! kill -0 "$SERVER_PID" 2>/dev/null; then
            echo "[ERROR] vLLM server process ($SERVER_PID) died unexpectedly during initialization." >&2
            exit 1
        fi
...
run_eval_worker() {
    ...
    # evalhub.cli always runs in the stable global environment
...
# ------------------------------------------------------------------------------
run_orchestrator() {
    echo "[INFO] Initializing Unified Pipeline Orchestrator (vLLM Monolithic Mode)"
